...
## 2026-07-01 — Card editor stage propagation

- Fixed card editor stage saves so modal edits use the same stage-driven progress path as drag/drop moves, keeping EVM, reports, completion counts, and activity synchronized.
- Added focused browser QA coverage and restored the `pnpm run release` static validation/public-build refresh path without adding dependencies.
