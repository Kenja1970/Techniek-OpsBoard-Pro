...
### Fixed — card editing reliability
- Saving a card after changing its Stage now applies the same stage-derived percent-complete logic used by drag/drop, so stale Progress field values cannot block Earned Value, project rollups, resource allocation, and reports from updating with the destination stage.

### QA / QC
- Suite expanded to **227 checks (227/227 passing)** with a focused card-editor stage-save regression check.
- Added a zero-dependency `pnpm run release` wrapper that validates guardrails and refreshes the repository-root `opsboard/` static public build.
