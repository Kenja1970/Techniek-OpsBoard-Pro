...
A browser-based red-team suite lives in [`tests/qa.html`](tests/qa.html) (open it to run). It independently re-derives every financial/EVM/resource metric from raw data and asserts PMI reactivity — that creating, moving, and editing cards cascades into rollups, EVM (project **and** program), resources, the portfolio, and the weekly trend. Current status: **227/227 checks passing** — see [`docs/qa/QA-REPORT.md`](docs/qa/QA-REPORT.md).
