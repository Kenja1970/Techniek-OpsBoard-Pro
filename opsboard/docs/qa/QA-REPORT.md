...
**Version:** 2.5.1
**Result:** ✅ **227 / 227 checks passed · 0 failures**
...
| 8i.1 | Card editor stage save keeps reports stage-driven | 3 | ✅ 3/3 |
...
v2.5.1 passes **227/227** with no app runtime failures observed. Local browser preview showed only a benign missing-favicon 404. This round fixes card-editor stage saves so changing Stage applies the same stage-derived percent-complete used by drag/drop, preventing stale Progress input values from blocking EVM/report propagation. The suite is committed so every future change (including the nightly routine) can be re-validated.
