...
**Version:** 2.5.1
**Result:** ✅ **229 / 229 checks passed · 0 failures**
...
| 8b.1 | Card editor stage save updates progress + EVM/resources | 5 | ✅ 5/5 |
...
- **Move a card to *any* stage** → its percent-complete is set by stage position (first 0% → last 100%), so **Earned Value, the project rollup, the client billing snapshot, completion counts, and the dashboard trend all move with it** — not only when it reaches Done (groups 6, 8b).
- **Save a changed stage from the card editor** → the same propagation path applies, so modal edits cannot leave percent complete, EVM, or resource allocation stale (group 8b.1).
...
v2.5.1 passes **229/229** with zero console errors. This round adds focused editor-stage propagation checks so card modal saves, drag/drop moves, resource allocation, and reports stay synchronized. The suite is committed so every future change (including the nightly routine) can be re-validated.
