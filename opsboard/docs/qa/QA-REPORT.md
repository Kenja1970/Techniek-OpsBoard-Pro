...
**Version:** 2.5.1
**Result:** ✅ **228 / 228 checks passed · 0 failures**
...
| 8b.1 | Card editor stage save updates progress + EVM | 4 | ✅ 4/4 |
...
- **Save a changed stage from the card editor** → the same propagation path applies, so modal edits cannot leave percent complete or EVM stale (group 8b.1).
...
v2.5.1 passes **228/228** with zero console errors. This round adds a focused editor-stage propagation check so card modal saves, drag/drop moves, and reports stay synchronized. The suite is committed so every future change (including the nightly routine) can be re-validated.
