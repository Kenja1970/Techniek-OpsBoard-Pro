...
(function () {
  ...
  function boardById(id) { return state.boards.filter(function (b) { return b.id === id; })[0] || null; }
  ...
  function moveCard(cardId, colId, beforeCardId) {
    ...
    mutate(function () {
      ...
      if (changedCol) setCardStage(c, b, colId);
      else c.columnId = colId;
    });
  }
  function setCardStage(c, b, colId) {
    var changedCol = c.columnId !== colId;
    c.columnId = colId;
    if (!changedCol) return false;
    ...
    foot.push({ label: c._new ? "Create card" : "Save", cls: "btn primary", fn: function () {
      ...
      mutate(function () {
        saveCardFields(c, b, {
          ...
          due: $("#fDue").value || null,
          estimateHours: parseFloat($("#fEst").value) || 0,
          loggedHours: parseFloat($("#fLogged").value) || 0,
          progress: clamp(parseInt($("#fProgress").value, 10) || 0, 0, 100),
  ...
  function saveCardFields(c, b, fields) {
    var wasNew = !!c._new;
    var nextColId = fields.columnId || c.columnId;
    var stageChanged = wasNew || c.columnId !== nextColId;
    c.title = fields.title;
    c.desc = fields.desc;
    c.assigneeId = fields.assigneeId || null;
    c.priority = fields.priority;
    c.type = fields.type;
    c.projectId = fields.projectId || null;
    c.due = fields.due || null;
    c.estimateHours = fields.estimateHours || 0;
    c.loggedHours = fields.loggedHours || 0;
    if (stageChanged) {
      setCardStage(c, b, nextColId);
      c.progress = stageProgress(b, nextColId);
    } else {
      c.progress = clamp(fields.progress || 0, 0, 100);
    }
    c.milestone = !!fields.milestone;
    c.labels = fields.labels || [];
    c.checklist = fields.checklist || [];
  ...
  window.TechniekOpsBoard = {
    ...
    _qa: {
      ...
      editCardRaw: function (cardId, fields) {
        var c = cardById(cardId), b = boardById(c.boardId), f = fields || {};
        saveCardFields(c, b, {
          title: f.title != null ? f.title : c.title,
          desc: f.desc != null ? f.desc : c.desc,
          columnId: f.columnId != null ? f.columnId : c.columnId,
          assigneeId: f.assigneeId != null ? f.assigneeId : c.assigneeId,
          priority: f.priority != null ? f.priority : c.priority,
          type: f.type != null ? f.type : c.type,
          projectId: f.projectId != null ? f.projectId : c.projectId,
          due: f.due != null ? f.due : c.due,
          estimateHours: f.estimateHours != null ? f.estimateHours : c.estimateHours,
          loggedHours: f.loggedHours != null ? f.loggedHours : c.loggedHours,
          progress: f.progress != null ? f.progress : c.progress,
          milestone: f.milestone != null ? f.milestone : c.milestone,
          labels: f.labels != null ? f.labels : (c.labels || []).slice(),
          checklist: f.checklist != null ? f.checklist : (c.checklist || []).slice(),
        });
        save();
        return c;
      },
