...
(function () {
  ...
  function applyCardStage(c, b, colId, beforeCardId, forceReorder) {
    var changedCol = c.columnId !== colId;
    c.columnId = colId;
    if (changedCol || forceReorder) {
    ...
    // Stage drives percent-complete so editor saves and drag/drop moves both flow
    // into progress, earned value, completion counts, and all reports.
    if (changedCol) {
  ...
  function moveCard(cardId, colId, beforeCardId) {
    ...
    var b = activeBoard();
    mutate(function () {
      applyCardStage(c, b, colId, beforeCardId, true);
    });
  }
  ...
  function openCardEditor(cardId) {
    ...
    foot.push({ label: c._new ? "Create card" : "Save", cls: "btn primary", fn: function () {
      ...
      mutate(function () {
        ...
        var stageChanged = applyCardStage(c, b, $("#fCol").value, null, false);
        if (wasNew) { delete c._new; logActivity(c, "Card created"); state.cards.push(c); }
        else if (!stageChanged) logActivity(c, "Card updated");
      });
      closeModal();
  ...
  window.TechniekOpsBoard = {
    ...
    _qa: {
      ...
      editCardStageRaw: function (cardId, colId) { var c = cardById(cardId); var b = state.boards.filter(function (x) { return x.id === c.boardId; })[0]; applyCardStage(c, b, colId, null, false); save(); return c; },
