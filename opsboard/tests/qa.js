...
(function () {
  ...
  function run() {
    ...
    group("8b.1 · Card editor stage save updates progress + EVM");
    Q.resetDemo();
    (function () {
      var card = Q.state().cards.filter(function (c) { return c.projectId && (c.estimateHours || 0) > 0; })[0];
      var bid = card.boardId, pid = card.projectId;
      var cols = Q.columnIds(bid), n = cols.length;
      var targetIdx = Math.max(1, Math.floor((n - 1) / 2));
      Q.moveCardRaw(card.id, cols[0]);
      var ev0 = Q.projectEVM(pid).ev;
      var saved = Q.editCardStageRaw(card.id, cols[targetIdx]);
      var expected = Q.stageProgress(bid, cols[targetIdx]);
      check("editor save changed stage", saved.columnId === cols[targetIdx]);
