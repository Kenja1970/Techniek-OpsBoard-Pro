...
(function () {
  ...
  function run() {
    ...
    /* ---- 8i.1. Card editor stage save should match drag/drop stage logic ---- */
    group("8i.1 · Card editor stage save keeps reports stage-driven");
    Q.resetDemo();
    (function () {
      var card = Q.state().cards.filter(function (c) {
        var cols = Q.columnIds(c.boardId);
        return c.projectId && (c.estimateHours || 0) > 0 && c.columnId !== cols[cols.length - 1];
      })[0];
      if (!card) { check("fixture available", false, "no editable non-done project card"); return; }
      var cols = Q.columnIds(card.boardId);
      var target = cols[cols.length - 1];
      var expected = Q.stageProgress(card.boardId, target);
      var ev0 = Q.projectEVM(card.projectId).ev;
      Q.editCardRaw(card.id, { columnId: target, progress: 5 });
      var edited = Q.cardById(card.id);
      check("stage field saved", edited.columnId === target);
      check("stale progress input ignored on stage change", edited.progress === expected, "got " + edited.progress + " exp " + expected);
      check("project EV follows edited stage", Q.projectEVM(edited.projectId).ev > ev0, "Δ " + Math.round(Q.projectEVM(edited.projectId).ev - ev0));
    })();
