...
(function () {
  ...
  function run() {
    ...
    (function () {
      ...
      var evAtDone = Q.projectEVM(pid).ev;
      Q.editCardStageRaw(card.id, cols[0]);
      var ce = Q.state().cards.filter(function (c) { return c.id === card.id; })[0];
      check("editor stage save → stageProgress", ce.columnId === cols[0] && ce.progress === 0, "stage " + ce.columnId + " progress " + ce.progress);
      check("editor stage save moves EV", Q.projectEVM(pid).ev < evAtDone, "EV " + Math.round(Q.projectEVM(pid).ev) + " from " + Math.round(evAtDone));
